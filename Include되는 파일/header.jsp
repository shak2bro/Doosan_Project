...
<!--////////// script import BEGIN //////////-->
	...
	<link rel="stylesheet" type="text/css" href="<c:url value='/cmm/css/new.mobile.css' />" />
	
	<!-- <link rel="stylesheet" type="text/css" href="<c:url value='/cmm/js/jquery/jquery.mobile-1.1.1.css' />" /> -->
...
<!--////////// script import END //////////-->
	...
	if(!($.browser.msie)){
		/* origin Source */
		var myScroll;
		function loaded() { myScroll = new iScroll('wrapper', {useTransform: false, onBeforeScrollStart: null, momentum: false}); }
		document.addEventListener('touchmove', function (e) { e.preventDefault(); }, false);
		//document.addEventListener('DOMContentLoaded', setTimeout(function () { loaded(); }, 200), false);*/
		document.addEventListener('DOMContentLoaded', function () { setTimeout(loaded, 200); }, false);

		/* Lasted iscroll Source */
		/*var myScroll;
		function loaded() {	myScroll = new iScroll('wrapper');}
		document.addEventListener('touchmove', function (e) { e.preventDefault(); }, false);
		document.addEventListener('DOMContentLoaded', function () { setTimeout(loaded, 200); }, false);*/

		/* other iscroll Source */
		/*function loaded() {
			document.addEventListener('touchmove', function(e){ e.preventDefault(); });
			myScroll = new iScroll('scroller', {useTransform: true, onBeforeScrollStart: null});
		}
...
<!-- origin : master Javascript // SYSTEM Application -->
	<script type="text/javascript">		
		jQuery(document).ready(function() {				
			...
			//loadpage();
			//alert($('#gingerCss').attr('href'));

			


			// if(isMobile.Android()){ // Your mobile OS is Android,
			// 	if(Math.floor(androidversion) == "2"){ // your Mobile OS Version is 2.X(Or GINGERBREAD)
		...
		function inputNumCom(obj, id){
			...
			if( !((48 <= keycode && keycode <=57) || keycode == 13 || keycode == 46) ){
				...
				$(id).find('div.content').css({
					'text-align':'center',
					'font-weight':'bold',
					'font-size':'1.4em' // [em] Tag is [X.X] lower or higher even.
				}).text('只能输入数字。'); //transition text
				//event.keyCode = 0;  // Event cancel
			}
		}
		...
		} // chinese simpled input type change.
		...
		function popDim(id, url){			
			...
			if(id != undefined){
				...
				// Height is not fix. So, this height is flexable.
...
<!-- orgin : tagInc // Content Application -->
	<script type="text/javascript">
		...
		*/
		...
		// List style is BoxModel without List type. So, specially list type is switch other pages.
		function addClasses(){
			var locSrc = String(window.location);
			locSrc = locSrc.substr(locSrc.lastIndexOf("/")+1);
			locSrc = locSrc.split(".do",1);

			if(locSrc == "cmAreaSerchR" || locSrc == "ciCustLstL" || locSrc == "ciSilimarLstL" || locSrc == "rmRlprLstL" || locSrc == "scSChncLstL" || locSrc == "sfCustLstL" || locSrc == "ssSChncLstL" || locSrc == "ssSChncWhlLstL" || locSrc == "csCustLstL" || locSrc == "mainpage" || locSrc == "csCustInfoSelt" || locSrc == "stStatMenuL"){ 
				//Behind, specially list type is add by "or-type"
		...
		// 	if(Math.floor(androidversion) == "2"){ // only Android Version 2 then
		// 		if($('div.selectGroup').attr('class')){ // Login Page > difference Login Page class
		// 			var idx = $('div.selectGroup select').index('div.selectGroup select');
		// 			$('div.selectGroup select').css('width','150px');
		// 			$('div.selectGroup select').each(function(idx){
		// 				//$('head').html('111');
		// 				//$(this).eq(idx).find('option').text('111');
		// 				$(this).eq(idx).prev().css('display','none')
		// 				$(this).eq(idx).selectmenu();
		// 			});
		// 		}else{
		// 			var idx = $('div#contentBoxWrap select').index('div#contentBoxWrap select');
		// 			$('div#contentBoxWrap select').css('width','150px');
		// 			$('div#contentBoxWrap select').each(function(idx){
		// 				$(this).eq(idx).prev().css('display','none')
		// 				$(this).eq(idx).selectmenu();
		// 			});
		// 		}
		...
		/*function selectBox(){
			...
			$(this).eq(idx).select2();
				...
				var selectText = $('div#contentBoxWrap select').eq(idx).find('option:selected').text();
				var viewText = $('div.select2-container a span').eq(idx);
				var firstText = $('div#contentBoxWrap select').eq(idx).find('option').text();
				if(selectText == "" || selectText == null){
					//alert($('div#contentBoxWrap select[id^="callonType"]').eq(idx).find('option:selected').text()+":"+ idx);
					if($('div#contentBoxWrap select').eq(idx).attr('id') == "trendPhase"){
						var currentIdx = idx;
						$('div.select2-container a span').eq(currentIdx).text("N/A 未明确需求");
					}else{
						$('div.select2-container a span').eq(idx).text("--请选择--");
					}
				}else{
					viewText.text(selectText);
				}
			});	
...
<body>
	...
	<!-- Specially page is not footer on screen. So, You must add class["noFoot"] at division tag id[wrapper]  -->	
	<!-- mainpage -->
